...
describe("fetchVault — STAFFD self override (W91.5)", () => {
  ...
  it("non-super-admin gets their businesses row (regression — customer path unchanged)", async () => {
    ...
    const v = await fetchVault("tok", "user-1");
    expect(v!.business_name).toBe("Acme Co");
    expect(v!.brand_voice).toBe("friendly");
  });

  it("a clientId (agency mode) still loads the client vault, not STAFFD self", async () => {
    adminMock.isOperator = true; // even as operator, an explicit client view wins
    vi.stubGlobal("fetch", vi.fn(async (url: string) =>
      url.includes("/clients/")
        ? { ok: true, json: async () => ({ agency_user: "admin", name: "Client X" }) }
        : { ok: true, json: async () => ({ items: [] }) },
    ));
    const v = await fetchVault("tok", "admin", { clientId: "c1" });
