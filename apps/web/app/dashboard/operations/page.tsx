...
const QUICK_ACTIONS = [
  ...
  { label: "Project brief", prompt: "Draft a clear project brief for a new initiative or task I'm kicking off." },
  ...
  async function run(customTask?: string) {
    ...
    try {
      const res = await fetch("/api/agent", {
        ...
        body: JSON.stringify({ task: finalTask, department: "operations", userId, pbToken }),
      });
      ...
      <div className="relative z-10 w-full max-w-3xl mx-auto px-6 py-8 flex flex-col flex-1">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <h1 className="font-bold" style={{ color: "#F0F0F8", fontSize: "1.5rem", lineHeight: 1.2, letterSpacing: "-0.01em" }}>Operations</h1>
            </div>
          </div>
          <p className="text-sm" style={{ color: "#9090A8" }}>Your AI operator builds processes, SOPs, and workflows that keep your business running.</p>
        </div>
        ...
        <div className="rounded-2xl overflow-hidden mb-5" style={{ background: "#111118", border: "1px solid #2A2A38" }}>
          <textarea value={task} onChange={(e) => setTask(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && (e.metaKey || e.ctrlKey)) run(); }}
            placeholder="Describe what you need — e.g. 'SOP for client onboarding' or 'weekly team meeting agenda'…"
          ...
          <div className="rounded-2xl overflow-hidden flex-1" style={{ background: "#111118", border: "1px solid #2A2A38" }}>
            <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: "1px solid #1E1E2A" }}>
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-xs font-semibold" style={{ color: "#9090A8" }}>The Operator</span>
