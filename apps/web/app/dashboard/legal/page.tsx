...
const QUICK_ACTIONS = [
  { label: "NDA draft", prompt: "Draft a simple mutual non-disclosure agreement for a new business partnership." },
  { label: "Terms of service", prompt: "Write a Terms of Service for my business website." },
  { label: "Privacy policy", prompt: "Write a Privacy Policy for my business website." },
  { label: "Contractor agreement", prompt: "Draft a simple contractor/freelancer agreement for a short-term project." },
  { label: "Refund policy", prompt: "Write a clear refund and cancellation policy for my business." },
  { label: "Late payment notice", prompt: "Write a formal late payment notice letter to send to an overdue client." },
];
  ...
  async function run(customTask?: string) {
    ...
    try {
      const res = await fetch("/api/agent", {
        ...
        body: JSON.stringify({ task: finalTask, department: "legal", userId, pbToken }),
      });
      ...
      <div className="relative z-10 w-full max-w-3xl mx-auto px-6 py-8 flex flex-col flex-1">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl" style={{ background: "rgba(91,33,232,0.15)", border: "1px solid rgba(91,33,232,0.25)" }}>⚖️</div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest mb-0.5" style={{ color: "#5B21E8" }}>Department</p>
              <h1 className="font-bold" style={{ color: "#F0F0F8", fontSize: "1.5rem", lineHeight: 1.2, letterSpacing: "-0.01em" }}>Legal</h1>
            </div>
          </div>
          <p className="text-sm" style={{ color: "#9090A8" }}>Your AI counsel drafts contracts, policies, and agreements for your business.</p>
        </div>
        ...
        <div className="rounded-2xl overflow-hidden mb-5" style={{ background: "#111118", border: "1px solid #2A2A38" }}>
          <textarea value={task} onChange={(e) => setTask(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && (e.metaKey || e.ctrlKey)) run(); }}
            placeholder="Describe what you need — e.g. 'NDA for a new supplier' or 'refund policy for a digital product'…"
          ...
          <div className="rounded-2xl overflow-hidden flex-1" style={{ background: "#111118", border: "1px solid #2A2A38" }}>
            <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: "1px solid #1E1E2A" }}>
              <div className="flex items-center gap-2.5">
                <div className="w-6 h-6 rounded-lg flex items-center justify-center text-sm" style={{ background: "rgba(91,33,232,0.15)" }}>⚖️</div>
                <span className="text-xs font-semibold" style={{ color: "#9090A8" }}>The Counsel</span>
                {loading && <span className="flex items-center gap-1.5 text-xs" style={{ color: "#5A5A70" }}><span className="inline-block w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: "#5B21E8" }} />drafting</span>}
              </div>
