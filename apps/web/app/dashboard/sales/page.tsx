...
const QUICK_ACTIONS = [
  { label: "Cold outreach", prompt: "Write a cold outreach email to a potential new client." },
  { label: "Follow-up", prompt: "Write a follow-up email to a prospect I haven't heard back from." },
  { label: "Proposal outline", prompt: "Draft a short proposal outline for a new client engagement." },
  { label: "Objection response", prompt: "Write a response to the objection: 'Your price is too high.'" },
  { label: "LinkedIn message", prompt: "Write a short LinkedIn connection message to a potential client." },
  { label: "Meeting prep", prompt: "Give me a prep guide for an upcoming sales call with a new prospect." },
];
  ...
  async function run(customTask?: string) {
    ...
    try {
      const res = await fetch("/api/agent", {
        ...
        body: JSON.stringify({ task: finalTask, department: "sales", userId, pbToken }),
      });
      ...
      <div className="relative z-10 w-full max-w-3xl mx-auto px-6 py-8 flex flex-col flex-1">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <h1 className="font-bold" style={{ color: "#F0F0F8", fontSize: "1.5rem", lineHeight: 1.2, letterSpacing: "-0.01em" }}>Sales</h1>
            </div>
          </div>
          <p className="text-sm" style={{ color: "#9090A8" }}>Your AI closer knows your business. Tell it what to write.</p>
        </div>
        ...
        <div className="rounded-2xl overflow-hidden mb-5" style={{ background: "#111118", border: "1px solid #2A2A38" }}>
          <textarea value={task} onChange={(e) => setTask(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && (e.metaKey || e.ctrlKey)) run(); }}
            placeholder="What do you need? Describe a prospect, situation, or goal…"
          ...
          <div className="rounded-2xl overflow-hidden flex-1" style={{ background: "#111118", border: "1px solid #2A2A38" }}>
            <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: "1px solid #1E1E2A" }}>
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-xs font-semibold" style={{ color: "#9090A8" }}>The Closer</span>
