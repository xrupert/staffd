...
const QUICK_ACTIONS = [
  ...
  { label: "Business health check", prompt: "Give me an honest assessment of where my business is strong and where it has real gaps." },
...
export default function CEOPage() {
  ...
  async function run(customTask?: string) {
    ...
    try {
      const res = await fetch("/api/agent", {
        ...
        body: JSON.stringify({ task: finalTask, department: "ceo", userId, pbToken }),
      });
  ...
  return (
    <main className="min-h-screen flex flex-col" style={{ background: "#09090F" }}>
      ...
      <div className="fixed pointer-events-none" style={{ top: "-150px", left: "50%", transform: "translateX(-50%)", width: "900px", height: "600px", borderRadius: "50%", background: "radial-gradient(ellipse, rgba(91,33,232,0.14) 0%, transparent 65%)" }} />
      ...
      <div className="relative z-10 w-full max-w-3xl mx-auto px-6 py-8 flex flex-col flex-1">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-3">
            <div
              ...
              style={{
                background: "linear-gradient(135deg, rgba(91,33,232,0.25) 0%, rgba(124,79,240,0.15) 100%)",
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest mb-0.5" style={{ color: "#5B21E8" }}>Strategy</p>
              <h1 className="font-bold" style={{ color: "#F0F0F8", fontSize: "1.5rem", lineHeight: 1.2, letterSpacing: "-0.01em" }}>The CEO</h1>
            </div>
          </div>
          <p className="text-sm" style={{ color: "#9090A8" }}>Your AI strategic advisor. Cuts through noise, connects the dots across your business, and tells you what actually matters.</p>
        </div>
        ...
        {(output || loading) && (
          <div className="rounded-2xl overflow-hidden flex-1" style={{ background: "#111118", border: "1px solid rgba(91,33,232,0.2)" }}>
            <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: "1px solid #1E1E2A" }}>
              <div className="flex items-center gap-2.5">
                <div className="w-6 h-6 rounded-lg flex items-center justify-center text-sm" style={{ background: "rgba(91,33,232,0.2)" }}>🎯</div>
                <span className="text-xs font-semibold" style={{ color: "#9090A8" }}>The CEO</span>
                {loading && <span className="flex items-center gap-1.5 text-xs" style={{ color: "#5A5A70" }}><span className="inline-block w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: "#5B21E8" }} />thinking</span>}
              </div>
        ...
        {!output && !loading && (
          <div className="mt-auto pt-8 pb-4">
            <a href="/dashboard/vault" className="flex items-center gap-3 text-xs group" style={{ color: "#3A3A55", textDecoration: "none" }}>
              <span>🔐</span>
              <span className="group-hover:text-purple-400 transition-colors">Fill in your Business Vault and the CEO advisor will give you sharper, more specific strategic advice →</span>
            </a>
          </div>
        )}
      </div>
