...
export default function AgentPage({
  ...
  async function loadContext() {
    ...
    // Load templates for this department
    try {
      const res = await pb.collection("templates").getList(1, 50, {
        filter: `user = '${userId}'`,
        sort: "name",
      });
      setTemplates(res.items.map((t) => ({ id: t.id, name: t.name as string, content: t.content as string })));
    } catch { /* templates collection may not exist yet */ }
  }
  ...
  const deptTemplates = templates.filter(
    (t) => !t.name.startsWith("__dept__") // all templates available across departments
  );
    ...
    <main className="min-h-screen flex flex-col no-print-chrome" style={{ background: "#09090F" }}>
      ...
      <div className="print-only print-header">
        {logoUrl && (
          ...
          <img src={logoUrl} alt={businessName} className="print-logo" />
        )}
      ...
      <div className="relative z-10 w-full max-w-3xl mx-auto px-6 py-8 flex flex-col flex-1">
        ...
        {deptTemplates.length > 0 && (
          <div className="flex items-center gap-2 mb-4 no-print">
            ...
            {showTemplates && (
              <div
                className="absolute z-20 mt-8 rounded-xl overflow-hidden shadow-xl"
        ...
        {error && (
          <div className="px-4 py-3 rounded-xl text-xs mb-4 no-print"
