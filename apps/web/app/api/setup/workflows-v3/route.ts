/**
 * POST /api/setup/workflows-v3 (wire-the-loop) — adds the follow-on-thinking
 * fields to the workflows collection:
 *   goal           (text) — the user-approved goal a planner workflow serves
 *   suggested_next (json) — up to 3 {title, goal} follow-on suggestions,
 *                           written by the drain when the workflow completes
 * Idempotent. Same shape/gating as workflows-v2.
 */
...
const NEW_FIELDS = [
  ...
  { name: "suggested_next", type: "json", required: false },
];
...
export async function POST() {
  ...
  try {
    ...
    console.error("workflows-v3 setup error:", err);
