...
| Post W95.7.3a (brand-voice regression closure — GATES L4) | 929/930 | Fixed the W95.7.1 state-coupling regression: hiding the ClientSwitcher orphaned `staffd_active_client` (still read by 7 sites) → operators ran in agency-client mode → `fetchVault` returned the sparse client vault and exited before the STAFFD_SELF override → every specialist call lost brand voice (W95.7.2 forensic). FIX (ratified): (B, load-bearing) reordered `fetchVault` so the super-admin STAFFD_SELF override runs BEFORE the clientId branch — operator identity is absolute, clientId can never shadow it; non-operator agency users still get clientId routing (W94 path preserved). (Hygiene, Standard #30) `StaleClientStateCleanup` client-init in root layout clears the orphaned key on every route (idempotent; removed at W94). 7 read sites untouched (reads → null after cleanup, operator-bypassed at vault layer). State-coupling audit (Standard #31): `staffd_active_client` was the ONLY key in the orphaned-writer/live-reader class. +4 tests (929; 3 fetchVault invariants incl. the W91.5 test that pinned the bug, now inverted + 2 cleanup RTL). `/api/agent` + `packages/agents` untouched. |
| Post W73 / L4 Tranche 2 (plan preview/commit split) | 1037/1038 | Propose-then-ratify for L4 (consistent with the tier picker + confirm-to-commit). `POST /api/workflow/plan` is now PREVIEW — LLM decompose → parsePlan → returns the plan, persists NOTHING (so a multi-step workflow never spends the customer's agent calls before they approve). New `POST /api/workflow/commit` re-validates the client-approved plan through the SAME parsePlan trust boundary (a tampered preview can't create an unsound/oversized workflow) → materializes workflow + task DAG → the existing drain runs it. Also fixed a heavy-import smell surfaced by the test: the canonical `ALL_DEPTS` moved to the PURE `planner.ts` (was in the Anthropic-laden orchestrator handler), so lightweight routes import it without dragging in the SDK; the handler now imports it back (single source preserved, Standard #2). REPRIORITIZED the Tranche-1 "dedicated plan intent" debt: the ripple would force a `plan` case into the orchestrator handler-dispatch switch where a planner doesn't belong, for cosmetic cost attribution on a feature with zero live traffic — deferred until it has traffic to attribute. +3 tests (commit route; plan test now asserts preview persists nothing). Tranche 3 = the CommandCenter UI trigger (goal → preview the plan → approve → commit). |
| Post W73 / L4 Tranche 1 (workflow planner) | 1034/1035 | The brain that makes STAFFD an "automated team" (L3→L4). KEY INSIGHT: the execution substrate already existed (W71 drainTasks runs a task DAG honoring depends_on + retries; W72 reconcileWorkflow completes the parent + aggregates; W95.6.x review gates) — so L4 is purely "goal → workflows row + workflow_tasks DAG", reusing all execution unchanged. Built: `_lib/orchestrator/planner.ts` (PURE, fully tested — buildPlannerPrompt + parsePlan [the trust boundary: validates LLM JSON into a sound DAG — routable departments via the now-exported canonical ALL_DEPTS, non-empty tasks, dependsOn references EARLIER steps only ⇒ acyclic + topologically ordered, ≤MAX_PLAN_STEPS=12] + planToTaskSeeds) + `POST /api/workflow/plan` (owner-authed: LLM decompose → parsePlan → create parent workflow + tasks with depends_on resolved index→id → the existing per-minute drain runs the DAG). +13 tests (8 planner, 5 route incl. dependency-wiring + 401/400/502/422 guards). SURFACED DEBT (Tranche 2): the LLM call piggybacks the "synthesize" orchestrator intent (no new SDK site / no Record<OrchestratorIntent> ripple) — a dedicated `plan` intent+policy for correct cost attribution (#33) + a stronger model is next. Live decomposition quality + end-to-end drain run are operator-verified (queue #36) — not unit-coverable. Tranche 2 also: a UI trigger (CommandCenter "make this a workflow") + plan preview/approve. /api/agent untouched (route CALLS it via the existing drain). |
| Post W95.8 (notifications foundation — customer in-app) | 1016/1017 | First slice of the system→user notifications layer (ratified principle: notifications first-class at every tier). UNIFIED registry, not per-feature one-offs: `_lib/notifications/events.ts` (pure typed-event registry — audience × severity × render; zero vendor names) + `notifyUser` (server producer, best-effort, customer-audience persists / operator-audience routes elsewhere) + `notifications` collection (USER_OWNED; setup route + MIGRATION_REGISTRY + EXPECTED_COLLECTIONS=40) + `NotificationBell` (dashboard header inbox: unread badge, dropdown, mark-read straight to PB via USER_OWNED rules). First producer wired: `generation.ready` in completeJob's newly-completed transition (NOT the idempotent re-poll → no duplicates); the ephemeral in-thread "video ready" now also persists. `generation.failed` + `credits.low` defined in the registry, wired as fast-follow. +13 tests (6 events, 3 notify, 2 jobs producer, 2 bell). /api/agent + packages/agents untouched. Operator: run the `notifications` migration to light it up. |
| Post templates G0 closure | 1003/1004 | Investigation correction: templates was NOT half-built — UI + setup route + consumption (DepartmentRoom "Use a template" → agent USER TEMPLATE block) all already worked; the "no setup route" docs were stale. Real gap: setup route was unregistered in MIGRATION_REGISTRY (curl-only, health-invisible) → registered it + a drift-guard test (every registered migration targets an EXPECTED_COLLECTIONS name) + fixed the runbook. +2 tests. |
| Post W95.7.3d-h3 (health `_`-skip + catalog drift signal) | 1001/1002 | Two substrate fixes from the health/verify reports. (1) buildHealthReport now skips `_`-prefixed PB system collections (_mfas/_otps/_externalAuths/_authOrigins/_superusers) for found/missing/extra — they were false "drift" (never affected `ok`); mirrors verify-row-rules' `_` skip. (2) Catalog sync was blind to price/tier changes (margin risk #33) — added pure `computeCatalogDrift` (priceChanges/newModels/removedModels) + a structured `[catalog-drift]` cron log on risk-bearing change. Email/persisted-signal deferred to the shared super_admin_signals path (both crons), not bespoke here. +6 tests. NOTE: prior h1/h2 absolute counts were inflated; measured baseline is authoritative. |
| Post W95.7.3d-h2 (generation tier-gate invariant + inline picker) | 995/996 | HOTFIX H2, reframed by investigation. H2's dispatched premise — an "ungated specialist-driven generation path" — does NOT exist: the only two `runGeneration` call sites (CommandCenter, DepartmentRoom) already route through GenerationTierModal since Tranche 1 (the F5/F6 screenshot was pre-Tranche-1). So H2 became two real deliverables. (1) THE ENFORCED INVARIANT (durable value, Standard #38): `_lib/generation/trigger-surfaces.ts` registry + `trigger-surfaces.test.ts` guard — CI fails if any `runGeneration(` call site under app/ or lib/ is undeclared or stops referencing its paired tier-gate component, so a future ungated trigger (e.g. L4 workflow planner) can no longer slip in. (2) RATIFIED D2(a) UX: extracted `_lib/generation/tier-options.ts` (single source for picker rows — both surfaces render from `buildTierOptions`, Standard #2) + new `GenerationTierInline` (the gate rendered INLINE in the CommandCenter conversation stream under the chip that opened it, not as a full-screen overlay); DepartmentRoom keeps the overlay (room/form surface). +10 tests (4 tier-options, 3 trigger-surfaces invariant, 3 inline parity). **Absolute count is the MEASURED runner total (995 passing / 1 skipped); prior h1 row's `996` was an inflated bookkeeping figure — real pre-h2 baseline was 985, so this is +10, not a regression.** Generation still hard-fails live (`all_models_drifted`) until the operator runs the h1 catalog-sync — H2 hardens the gate, it does not restore generation. /api/agent + packages/agents untouched. |
| Post W95.7.3d-h1 (legacy slug fallback removed — fail loud) | 996/997 | HOTFIX H1 (F4 prod 404): removed `routeImageModel`/`routeVideoModel` hardcoded-slug fallback (`flux-dev-image`/bare `veo3` — absent from live catalog → 404). muapi route resolves the model EXCLUSIVELY via `resolveModel` → `routeFor` + `generation_models`: empty route → 500 `routing_unresolved`; first slug absent → fall to next; all absent → 500 `all_models_drifted` (both four-field structured bodies; never a hardcoded-slug 404). routing.ts `"flux-dev"`→`"flux-1-dev"` (DoD grep: zero legacy literals in app/lib outside tests). Runbook §6 fail-loud + §7 departments-without-entries. +6 tests (996). **H2 (specialist-driven tier-modal coverage) GATED**: investigation found all 3 runGeneration surfaces already route through GenerationTierModal (Tranche 1) — no ungated path exists; H2 premise unconfirmed, awaiting operator direction. /api/agent + packages/agents untouched. |
| Post W95.7.3d-T1 (three-tier credit-weight architecture) | 990/991 | Flat 1-credit/gen → THREE tiers (Quick/Pro/Premium). pricing.ts (locked bands video 4/8/60 $0.06–3.00, image 1/2/4 $0.01–0.30; computeTier/computeCreditWeight; round-up gap rule C3); routing.ts ((department,kind)→{defaultTier,models}; routeFor; validateRoutingSlugs C5 throws naming drift); catalog.ts + hourly cron muapi-catalog-sync → generation_models (ADMIN_ONLY, EXPECTED=39); GenerationTierModal (3 radios, dept default "✓ recommended", balance footer, locked tier weights); estimate route (dynamic-priced /estimate-cost → tier/weight); generation_jobs +tier/credit_weight/muapi_model (C4: defaults at the SINGLE write surface createJob, single read surface completeJob; nullable cols, no scattered fallbacks); muapi route gates+charges the tier WEIGHT (premium video=60 needs 60 credits). CommandCenter/DepartmentRoom chips open the picker → runGeneration(tier). Standards #36/#37 (gap at #35 intentional). +23 tests. /api/agent + packages/agents untouched. SUBSTITUTIONS surfaced: (a) charge = LOCKED tier weight (not per-USD estimate) so "customer sees cost before commit" is exact — estimate route serves catalog/transparency + future variable pricing; (b) modal shows weights immediately (no per-open estimate call) since weights are tier-locked. Routing slugs catalog-pending verification (validateRoutingSlugs surfaces any drift at sync). |
| Post W95.7.3c-b1 (Muapi margin fixes: dedup + completion webhook) | 967/968 | Cost-exposure fixes (Standard #33). Muapi billing = debit-on-completion (from published spec, NOT invoice — operator confirming). (1) Submit-time dedup: `generation_jobs.fingerprint` = sha256(userId\|kind\|prompt\|ratio); a matching PENDING job within a 15-min window is reused — BEFORE enrich + Muapi submit (closes multi-press leak; succeeded jobs never dedupe = legit re-gen). (2) Completion webhook (PRIMARY path; client-poll stays fallback): POST /api/generation/webhook, authed by an HMAC-derived capability token in `?token=` (raw `MUAPI_WEBHOOK_SECRET` never leaves server; timing-safe compare), looks up by `prediction_id`, pulls authoritative state via `checkPrediction`, runs the existing claim-first `completeJob` → closes the closed-tab leak (charge on completion even if client gone). submitPrediction passes `?webhook=` when the secret is set (else poll-only fallback). Standards #33 + #34 codified; billing policy documented in predictions.ts + muapi runbook. Cancel deferred (Q5/D). +17 tests. /api/agent + packages/agents untouched. Webhook is the additive completion path — W95.7.3b architecture intact (not inverted). |
| Post W95.7.3b (async Muapi generation — closes Inv 2+3) | 950/951 | sync→async image/video. The 60s server-side pollResult (the timeout source) DELETED: POST /api/integrations/muapi now submits + writes a `generation_jobs` row + returns 202 {jobId} (fast-path: charge+complete when Muapi returns the URL on submit, typical for images); the client polls GET /api/generation/[id]/status (one Muapi check/poll) until completed/failed. Credit = claim-first idempotent at completion-discovery (charge-on-success-only; ≤1 charge under re-poll). In-flight guard: CommandCenter `mediaBusyRef` (closes Inv 2 multi-press); DepartmentRoom keeps its loading-button UX. Shared `lib/generation-client.ts#runGeneration` (proposed as a hook; realized as a plain async fn — both call sites are imperative; surfaced to SA). Muapi HTTP primitives extracted to llm-free `_lib/integrations/muapi/predictions.ts` (avoids the Anthropic import footgun in the status route). New `generation_jobs` collection (USER_OWNED, migration + EXPECTED_COLLECTIONS=38 + GDPR cascade). +21 tests. Also de-flaked a date-fragile W92 usage-routes fixture (pre-existing; exposed by the 06-20 rollover, not this change). /api/agent + packages/agents untouched. Live sweep + operator smoke #30 (run `generation-jobs` migration first). |
| Post W95.7.1 (pre-L4 cleanup: clients hide + FC-2 → intent path) | 925/926 | Clients UI hidden pending W94 (dashboard nav link + ClientSwitcher removed; `/dashboard/clients` → `notFound()`); `clients` collection + GDPR cascade untouched. FC-2 action buttons migrated off operator-wide `/api/integrations/*` (which 403 for customers post-Standard #22) onto the per-customer confirm-to-commit intent path: new `IntentActionModal` wrapper (DepartmentRoom) + CommandCenter reuses its inline `setPendingIntents`; shared `FC2_ACTION_INTENT` map (send_to_crm→create_contact, send_email_campaign→draft_campaign, send_for_signature→send_for_signature). `open_support_ticket` / reputation "Send as Ticket" retired (hidden) — creating a NEW support conversation needs a future `create_support_thread` intent (reported). All 4 `/api/integrations/{twenty,listmonk,chatwoot,docuseal}` routes annotated operator-only (Standard #22); ZERO customer-facing callers remain. +12 tests. tsc + build clean. |
| **Post W95.7 (V1 SUBSTRATE COMPLETE — `00b30c0`)** | **913/914** | **V1 substrate marked shippable.** Cumulative: **37 collections · 13 intents · 17 migrations · 11 workers · 2 recipes · 5 vendor backends.** Front Desk audit (Standard #9): all 7 cards now per-customer — Email Campaigns repointed off operator-wide `integrations/listmonk` onto new per-customer `/api/front-desk/campaigns` + `ListmonkClient` campaign methods (list-scoped leak-guard: drafts/sends target ONLY `staffd-<userId>`, foreign campaigns invisible); card + drill-in compose/send repointed, list-picker dropped (implicit own list). Brand-voice: CI test (`brand-voice.test.ts`, comment/URL-skipping word-boundary scan of customer-facing surfaces) + fixed the one real leak ("Ticket opened in Chatwoot." → "Support ticket opened."). Plausible admin nit: auth-before-validation (anon empty POST now 403, not 400). `GET /api/admin/health` + `/dashboard/admin/health` (pure `buildHealthReport` over the canonical registries + one PB collections probe → collections/intents/workers/migrations/recipes/vendors all-green/red). Docs: ARCHITECTURE.md §0 V1 marker; STANDARDS #29 (doc/code drift-guard); SECOND_WORKER exported for introspection. +25 tests. Live sweep deferred to operator queue. |
| Post W95.6.y (Plausible read + operator provisioning) | 888/889 | SITE-PER-CUSTOMER (operator provisions; CE has no Sites API). `PlausibleClient` leak-guard (`forCustomer("")` throws; `siteIdFor` reads businesses.plausible_site_id via admin token; stats THROW when no site → never a cross-tenant leak; raw `pl()` module-private; `hasSiteFor`/`configured` statics); businesses-v3 migration (plausible_site_id, detectField). Operator provisioning: POST/DELETE /api/admin/plausible/[userId] (super-admin, writes businesses + audit row); usage roster carries plausibleSiteId; admin Users tab ● dot + inline Save/Clear. Customer: GET /api/front-desk/analytics (hasSite empty state) → Front Desk Site Analytics card repointed per-customer + /dashboard/front-desk/analytics drill-in (4 metric cards + inline-SVG sparkline + Top pages/sources + 7d/30d toggle, zero vendor name). PARADIGM.md "Recipe-driven workflow assembly" section (doc↔SECOND_WORKER drift guard). +21 tests. Live CE probe DEFERRED (no super-admin token + Sensitive PLAUSIBLE_* creds) |
| Post W95.6.x (Chatwoot writes + review step) | 869/870 | Workflow REVIEW STEP (foundational): workflows gains awaiting_review/cancelled statuses + review_required + draft_output (workflows-v2 migration); reconcileWorkflow pauses review_required workflows at awaiting_review on draft completion (no aggregate/auto-send), freezes awaiting_review/cancelled, resumes post-approve. 3 Chatwoot intents (reply_to_ticket delegate+review; resolve_ticket + tag_conversation direct; all autopilot "never"); 3 workers (chatwoot_resolve/_tag/_send, send reads reviewed draft + tombstone-on-cancel); docuseal_send activated (reads draft, tombstone-on-cancel); send_for_signature RETROACTIVELY review_required (now 1 task, docuseal enqueued on approve). approve/cancel endpoints (/api/workflows/[id]/[action]). Drafts card + /dashboard/front-desk/drafts (editable draft + Approve&Send/Cancel); inbox Reply button enabled → draft modal. ChatwootClient write methods. +35 tests. Live probe DEFERRED (gates production reply use) |
| Post W95.6 (Chatwoot inbox read) | 847/848 | ChatwootClient leak-guard (env creds + INBOX-PER-CUSTOMER partition: businesses.chatwoot_inbox_id, find-or-create with name-lookup race mitigation, raw cw() module-private); businesses-v2 migration (chatwoot_inbox_id, detectField); Support Inbox card → real per-customer data + drill Link; /dashboard/front-desk/inbox page (top-10 open + SideDrawer thread oldest-first + disabled Reply tooltip, Standard #21); GET /api/front-desk/inbox + /inbox/[id]. Read-only; zero vendor branding. Live probe DEFERRED (Standard #24). +13 tests |
| Post W95.5.1 (race fix + paradigm/standards docs) | 834/835 | Undo-mirror race guard: tombstone check in mirror_retry + listmonk_subscribe (skip if source row deleted/undone); twenty_update pushes CURRENT row state. docs/architecture/PARADIGM.md (registry-driven + append-only; catalogs 12 registries + 8 append-only logs) + STANDARDS.md (#1,2,7,8 recovered from code; #9,13-28 from dispatches; #3-6,10-12 flagged as not-in-repo for SA). ARCHITECTURE.md + README cross-ref. +7 tests (race fix). Docs-only otherwise |
